...
public class AiChatService {
    ...
    public String chat(String chatId, String userMessage) {
        ...
        try {
            ...
            log.error("[Agent] Error in agent loop: {}", e.getMessage());
            history.pollLast();
            return "Loi: " + e.getClass().getSimpleName() + ". Vui long thu lai.";
        }
    }
    ...
    private ArrayNode buildToolDefinitions() {
        ...
        fn.put("description",
                ...
                "You decide what command to run. Examples: 'docker ps', 'free -h', 'df -h', " +
                "'ps aux | grep java', 'ss -tlnp', 'docker logs --tail 20 <container>', " +
                "'systemctl status nginx', 'cat /etc/hosts', etc. " +
                "Only use safe, READ-ONLY commands. Never use: rm, kill, shutdown, dd, mkfs, etc.");
        ...
        cmdProp.put("description",
                ...
                "Must be a single-line, read-only command.");
    ...
    private String buildSystemPrompt() {
        return """
                ...
                COMMUNICATION:
                - ALWAYS respond in the SAME LANGUAGE the user writes in (Vietnamese ↔ English).
