...
| ⚡ [High-Speed File Transfer & Web Drop Portal](./file-transfer-portal.md) | Dual-Link LAN Gigabit & WAN Ngrok file transfer, < 30KB Zero-CDN Web Drop Portal, Zero-RAM 1MB streaming, RFC 7233 HTTP 206, QR code in-memory, 3-layer Zero-Disk Leak |
| 🎬 [Media Pipeline V2 (Video & 320kbps MP3)](./MEDIA_PIPELINE_ARCHITECTURE_V2.md) | Multi-platform media pipeline across 11+ social networks, Studio Master MP3 320kbps CBR multi-tier extraction, Telegram Native Audio Player card with waveform, and dual-track large file distribution |
...
| 🔌 [API Reference](./api-reference.md) | Complete REST API & Gateway reference: `/metrics`, `/auth`, `/files`, `/api/ai/transfer/*`, `/api/ai/media/*`, `/brain`, `/facebook`, `/tiktok`, `/v1` |
...
| 🛡 [Security Hardening & CodeQL Zero-Alerts](./security.md) | CodeQL 0-alert security hardening: PathSanitizer Barrier (`os.path.commonpath`), local directory enumeration (`iterdir()`), Anti-Reflective XSS, command sandboxing, and E2EE PIN protection |
...
4. **AI Agent Service (`ai-agent-service`):** FastAPI (Python 3.11) on Port `8084` & noVNC `6080` — Neuromorphic Brain Core (`BrainCore`), High-Speed Dual-Link File Transfer & Web Drop Portal, Multi-Platform Media Pipeline V2 (11+ MXH & 320kbps MP3), Multimodal Video Pipeline, Telegram Bot, 9Router LLM Pool, Playwright Facebook E2EE automation, TikTok streak keeper, and 32GB HDC Virtual Cortex. CodeQL 0-alert security certified.
