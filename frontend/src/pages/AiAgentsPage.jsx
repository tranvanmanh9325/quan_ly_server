...
} from '../components/agents/AgentUiControls';
import { useVncSession } from '../components/agents/useVncSession';
import AiAgentVncModal from '../components/agents/AiAgentVncModal';
import TikTokAgentTab from '../components/agents/TikTokAgentTab';
...
export default function AiAgentsPage() {
  ...
  // ── noVNC Session via custom hook ──────────────────────────────────────────
  const [ttRefreshKey, setTtRefreshKey] = useState(0);
  ...
  const vncStatusMsg = vnc.vncPlatform === 'facebook' ? vnc.vncStatusMsg : '';
  const vncIsLaunching = vnc.vncPlatform === 'facebook' && vnc.vncIsLaunching;
  ...
  return (
    <div style={{ padding: '24px 28px', overflowY: 'auto', height: '100%', boxSizing: 'border-box' }}>
      ...
      {/* ── Tab Content: TIKTOK SOCIAL & STREAKS AGENT ─────────────────────── */}
      {activePlatform === 'tiktok' && (
        <TikTokAgentTab
          onLaunchVnc={() => vnc.launchVnc('tiktok')}
          vncIsLaunching={vnc.vncPlatform === 'tiktok' && vnc.vncIsLaunching}
          vncStatusMsg={vnc.vncPlatform === 'tiktok' ? vnc.vncStatusMsg : ''}
          refreshTrigger={ttRefreshKey}
        />
      )}
      ...
      <AiAgentVncModal
        isOpen={vnc.showVncModal}
        platform={vnc.vncPlatform}
        vncUrl={vnc.vncUrl}
        vncStatusMsg={vnc.vncStatusMsg}
        isSaving={vnc.vncIsSaving}
        onClose={vnc.closeVnc}
        onSaveSession={vnc.saveSession}
      />
      </div>
    </div>
  );
